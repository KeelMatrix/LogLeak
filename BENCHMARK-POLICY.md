...
This normalized CPU criterion does not detect a change that slows the product workload and the reference workload identically, because that change can divide away. It also does not treat one fast observation as representative: only sustained regressions that move the median can fail the gate. The median is therefore the CPU pass/fail statistic, while the worst committed normalized sample plus fixed headroom remains the CPU threshold derivation rule. Sampled heap delta is an observation only; the product resource contract uses deterministic capture, payload, inspection-unit, and finding limits instead of process-wide heap accounting.
