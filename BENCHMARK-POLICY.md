...
These ten samples were recorded on Windows x64, target `net8.0`, .NET 8.0.31, Microsoft Windows 10.0.19045, process architecture X64, using the 100,000-event benchmark after the aggregate inspection guards were implemented. Raw values are also executable data in `tests/LogLeak.Probe.Runner/BenchmarkEvidence.cs`; the runner derives the summaries and thresholds from those values.
